...
1. Apply 'c$' to 25 targets where the changed region is different each time.
...
1. Intentionally run 'c$' at start, middle, and end of line contexts.
2. Test counts when supported (for example `2c$`) and compare boundaries.
3. Run one pass where you cannot use backspace in Insert mode.

## Mastery Drill (6 min)
1. Refactor a small snippet using only 'c$' plus motions and `.`.
