...
## Exercise List
1. `/EX-01` use `y{motion}` to copy/paste/move text into `EX-04` as appropriate.
2. `/EX-02` transfer text into `EX-05`.
3. `/EX-03` transfer text into `EX-06`.
4. Fill `EX-07`, `EX-08`, `EX-09` using three separate operations with `y{motion}`.
...
9. Repeat previous `y{motion}` effect with `.` when valid.
