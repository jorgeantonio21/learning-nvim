...
1. Apply 'cc' to 25 targets where the changed region is different each time.
...
1. Intentionally run 'cc' at start, middle, and end of line contexts.
2. Test counts when supported (for example `2cc`) and compare boundaries.
3. Run one pass where you cannot use backspace in Insert mode.

## Mastery Drill (6 min)
1. Refactor a small snippet using only 'cc' plus motions and `.`.
