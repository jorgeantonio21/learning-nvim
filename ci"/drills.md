...
1. Apply 'ci"' to 25 targets where the changed region is different each time.
...
1. Intentionally run 'ci"' at start, middle, and end of line contexts.
2. Test counts when supported (for example `2ci"`) and compare boundaries.
3. Run one pass where you cannot use backspace in Insert mode.

## Mastery Drill (6 min)
1. Refactor a small snippet using only 'ci"' plus motions and `.`.
